...
# File-based storage for latest wireframe (shared across workers)
import json
import os
from pathlib import Path

WIREFRAME_CACHE_FILE = Path(__file__).parent / "latest_wireframe.json"

def save_latest_wireframe(wireframe_data: dict):
    """Save latest wireframe to file for cross-worker sharing"""
    try:
        with open(WIREFRAME_CACHE_FILE, 'w') as f:
            json.dump(wireframe_data, f)
    except Exception as e:
        print(f"Error saving wireframe: {e}")

def load_latest_wireframe() -> dict:
    """Load latest wireframe from file"""
    try:
        if WIREFRAME_CACHE_FILE.exists():
            with open(WIREFRAME_CACHE_FILE, 'r') as f:
                return json.load(f)
    except Exception as e:
        print(f"Error loading wireframe: {e}")
    return {"components": [], "wireframe_id": None, "updated_at": None}
...
async def get_wireframes():
    ...
    """
    latest_wireframe = load_latest_wireframe()
